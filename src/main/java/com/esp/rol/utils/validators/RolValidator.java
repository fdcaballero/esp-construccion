package com.esp.rol.utils.validators;
...
public class RolValidator {
    ...
    private static void validateNombreisEmpty(String nombre) {
        if (nombre == null || nombre.trim().isEmpty())
            throw new ExceptionValidation("El nombre del Rol no puede ser nulo o vacío");
    }

    private static void validateNombreLength(String nombre, int maximo) {
        if (nombre.trim().length() > maximo)
            throw new ExceptionValidation("El nombre del Rol no puede exceder los " + LONGITUD + " caracteres");
    }
